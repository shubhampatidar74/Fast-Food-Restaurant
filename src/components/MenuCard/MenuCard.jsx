...
function MenuCard({ categorie}) {
  ...
  const { dataSaveLocal } = useContext(CardContext)

  const filteredFoodData =
    categorie === "All"
      ? foodData
      : foodData.filter((item) => item.categories === categorie);
  ...
  return (
    <>
      {filteredFoodData.map((item, index) => (
        <div
          key={index}
          className="mt-3 rounded-lg w-80 h-auto bg-gray-900 p-4 cursor-pointer"
        >
          <div className="w-full rounded-lg h-52 flex justify-center items-center bg-white">
            <img
              className="w-48 m-auto bg-transparent bg-opacity-50"
